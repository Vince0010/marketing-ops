...
export function calculateRiskScore(campaign: Record<string, unknown>): number {
  let score = 100
  if ((campaign.total_budget as number) < 1000) score -= 20
  const duration = new Date(campaign.end_date as string).getTime() - new Date(campaign.start_date as string).getTime()
