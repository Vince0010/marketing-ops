...
## Core Concept: Tasks vs Phases

> ⚠️ **Important Mental Model**
> 
> **Phases are workflow STAGES, not items that get "completed".**  
> **Tasks are the work items that flow through phases and have completion status.**

### What Gets Completed?

| Entity | Has `status`? | Gets "completed"? | Purpose |
|--------|--------------|-------------------|---------|
| **Task** (`MarketerAction`) | ✅ Yes | ✅ Yes | The actual work being done |
| **Phase** (`ExecutionPhase`) | ❌ Deprecated | ❌ No | A stage in the workflow (like a column) |

### Timeline Measurement

**The timeline is measured by how long a TASK spends in a certain phase**—NOT by marking phases as "complete".

```
Task: "Design hero banner"
├── Entered "Planning" phase at 9:00 AM
├── Exited "Planning" phase at 11:30 AM → 2.5 hours in Planning
├── Entered "Creative" phase at 11:30 AM
├── Exited "Creative" phase at 4:00 PM → 4.5 hours in Creative
├── Entered "Compliance" phase at 4:00 PM
└── Still in "Compliance" → timer running...
```

This task's journey through phases is recorded in `task_phase_history`, allowing you to:
- See how long each task spent in each phase
- Calculate aggregate time per phase (sum of all tasks)
- Detect drift (actual vs planned duration)

### Key IDs and Relationships

```
campaign_id (UUID)
    │
    ├── execution_phases (workflow columns)
    │       └── id (phase_id) ─────────────────┐
    │                                          │
    └── marketer_actions (tasks)               │
            ├── id (task/action_id)            │
            ├── campaign_id (FK)               │
            ├── phase_id (FK) ─────────────────┘  ← Links task to current phase
            ├── status: 'planned' | 'in_progress' | 'completed' | 'reverted'
            │           ↑ This is the TASK status, not phase status
            └── time tracking fields...
                    │
                    └── task_phase_history (movement log)
                            ├── action_id (FK to task)
                            ├── phase_id (FK to phase)
                            ├── entered_at
                            ├── exited_at (null if current phase)
                            └── time_spent_minutes
```

### Example: 3 Tasks in "Creative" Phase

```
Phase: "Creative" (id: abc-123)
├── Task A (id: task-001) → in_progress, 2h in phase
├── Task B (id: task-002) → completed, 4h in phase  
└── Task C (id: task-003) → in_progress, 1h in phase

Total time in Creative phase: 7 hours (aggregated from all tasks)
```

- **Task B is "completed"** even though it's in the Creative column—it finished its creative work
- **The phase "Creative" is NOT completed**—it's a permanent column that tasks flow through
- **Drift** = (7 hours actual) vs (planned phase duration) 
